...
Quest.playerReachedRequirements = 
	function(thePlayer, bOutput)
		if ( thePlayer:isQuestFinished(Quests[47]) ) then
			return true
		end
		return false
	end
...
Quest.onAccept =
    function(thePlayer)
                outputChatBox("Attent\\aeter: W\\aere super wenn du mir das schnell sagen k\\oenntest!", thePlayer, 255,255,255)
...
Quest.onFinish =
    function(thePlayer)
                ...
                outputChatBox("Attent\\aeter: Wenn du Zeit hast gibt es noch etwas zu erledigen.", thePlayer, 255,255,255)
        return true
    end
