...
Quest.Texts = {
	["Accepted"] = "Sammel 5 Pfandflaschen und gebe sie dem Obdachlosen!",
			...
			if (button == "left") and (state == "down") then
				if (getDistanceBetweenElements3D(v,thePlayer) < 5) then
					...
					if (QuestState and not(QuestState == "Finished")) then
						if (thePlayer:getInventory():removeItem(Items[2], 5)) then
...
Quest.onAbort = 
	function(thePlayer)
		...
		if (QuestState and (QuestState == "Finished")) then
			thePlayer:getInventory():addItem(Items[2], 5)
			thePlayer:refreshInventory()
		end
		return true
	end
