...
Quest.playerReachedRequirements = 
	function(thePlayer, bOutput)
		if ( thePlayer:isQuestFinished(Quests[49]) ) then
			return true
		end
		return false
	end
...
Quest.onAccept =
    function(thePlayer)
                outputChatBox("Attent\\aeter: Deine vorerst letzte Aufgabe.", thePlayer, 255,255,255)
...
Quest.onFinish =
    function(thePlayer)
                ...
                outputChatBox("Attent\\aeter: Vielen Dank. Mein Boss ist grade auf dem Weg nach Los Santos. Komm einfach mal vorbei wenn er angekommen ist.", thePlayer, 255,255,255)
        return true
    end
